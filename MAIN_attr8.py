'''
The authentication policy has eight attributes
'''
...
def main():
    ...
    fuzzy_pair = []
    key_pair = []
    bio = ['AAAAEEEEIIIINNNN','BBBBFFFFJJJJPPPP','CCCCGGGGLLLLQQQQ',
            'DDDDHHAHMMMMWWWW','AAAAEEAEIIIINNNN','BABBFFFFJJJJPPPP',
            'CCCCGGGGLLLLQQQQ','DDDDHHHHMMMMWWWW']
    ...
    policy_str = '((%s AND %s) AND (%s OR %s)) AND ((%s AND %s) OR (%s OR %s))' % (policy_str[0], policy_str[1], policy_str[2], policy_str[3], policy_str[4], policy_str[5], policy_str[6], policy_str[7])  # the policy can be changed. 
    ...
    fuzzy_key = []
    keys = []  
    fbio = ['AAAAEEEEIIIINNZZ','BBBBFFFFJJJJPPZZ','CCCCGGGGLLLLQQZZ',
            'DDDDHHAHMMMMWWZZ','AAAAEEAEIIIINNZZ','BABBFFFFJJJJPPZZ',
            'CCCCGGGGLLLLQQZZ','DDDDHHHHMMMMWWZZ']    
